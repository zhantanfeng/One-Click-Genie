...
1. 创建一条仅存在于内存中的待保存记录。
2. 启动录制服务。
3. 将本 App 移到后台，让用户进入目标界面。
4. 进入初始宽限期。

## 5.3 录制状态提示

录制期间不提供屏幕上的“停止”或“保存”按钮，结束条件统一为连续 3 秒没有新的触摸事件。
...
- 浮层不得消费或遮挡用户触摸。
- 每次检测到新的触摸事件时，3 秒倒计时重新开始。
- 浮层不计入录制轨迹。
- 初始宽限期内不因为无触摸而结束录制。

初始宽限期建议为 10 秒，用于给用户离开本 App 并进入目标页面。第一次有效触摸发生后，切换到正常的 3 秒空闲判定。
...
| `IDLE` | 正常显示记录列表 |
| `NAMING` | 输入新记录名称 |
| `RECORDING_GRACE_PERIOD` | 录制已准备，等待用户开始第一个操作 |
| `RECORDING` | 正在收集触摸事件 |
| `IDLE_COUNTDOWN` | 手指已抬起，等待连续 3 秒无新触摸 |
| `RECORDED_PREVIEW` | 录制结束但尚未保存 |
| `SAVED` | 记录已经持久化 |
| `PLAYBACK_COUNTDOWN` | 执行前 3 秒倒计时 |
| `PLAYING` | 正在回放轨迹 |
...
- 收到第一个有效触摸后退出初始宽限期。
- 完整收到 `ACTION_UP` 或最终 `ACTION_POINTER_UP` 后启动 3 秒倒计时。
- 倒计时期间如果收到新的 `ACTION_DOWN`，恢复录制并重新计时。
- 连续 3 秒没有任何有效触摸事件时结束录制。
- 正在触摸或长按时不得因为持续时间超过 3 秒而结束录制。
...
如果目标设备无法稳定边操作边采集，可采用全屏透明无障碍浮层：

1. 浮层捕获完整 `MotionEvent`。
2. 用户抬手后暂时移除或禁用浮层。
3. 立即通过 `dispatchGesture()` 将刚才的手势发送给目标 App。
4. 恢复浮层并继续等待下一次手势。

此方案可以稳定采集轨迹，但目标 App 只能在用户抬手后收到重建手势，拖动反馈不再与手指实时同步。因此它只能作为实验性兼容模式。
...
- `idleTimeoutMs = 3000`
- `initialGracePeriodMs = 10000`
- 初始宽限期在第一个有效 `ACTION_DOWN` 到来时结束。
- 3 秒计时从一个完整手势结束后开始。
- 触摸仍处于按下状态时不运行结束计时器。
...
如果用户点击后等待页面加载超过 3 秒，录制会提前结束。首版不通过页面加载检测解决该问题，也不提供超时配置。
...
| 3 秒内页面未加载完成 | 录制提前结束 | 作为已接受限制，首版不做页面识别 |
...
1. 原始触摸轨迹是唯一自动化模型，不引入业务语义动作。
2. 录制数据同时保留坐标、归一化坐标和相对时间。
3. 连续 3 秒无操作即结束，但正在进行的长按或手势不受影响。
4. 自动结束后必须由用户点击“保存”才写入正式记录。
5. 回放严格串行执行，并等待每个手势的系统完成回调。
